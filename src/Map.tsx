import { LatLng, Marker, UrlTile, Polyline, PROVIDER_DEFAULT } from 'react-native-maps'; // remove PROVIDER_GOOGLE import if not using Google Maps
import MapView from "react-native-map-clustering";
import { View, StyleSheet, Platform, Text } from 'react-native';
import { useState, useEffect } from 'react';
import Geolocation from '@react-native-community/geolocation';
import { Float } from 'react-native/Libraries/Types/CodegenTypes';
import json1 from "./narmada_composite/output.ts"
...
const PointMarker = ({index, point, location} : {index:any, point:Waypoint, location:any}) => {
...
export default () => {
    ...
    const [trackPoints, setTrackPoints] = useState<TrackPoint[]>(json1.trackPoints);
    ...
    return <View style={styles.container}>
        ...
        >
            {waypoints?.map((waypoint, index) => (
                <PointMarker key={index} index={index} point={waypoint} location={location} />
            ))}
